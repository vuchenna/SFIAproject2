...
def namegender():
    ...
    response2 = requests.get('http://service3:5000/name').text
    name = response2
    check = [name, gender]
...
def nameboy():
    response = requests.get('http://service2:5000/gender/boy').text
    boy = str(response)
    ...
    response2 = requests.get('http://service3:5000/namebyboy').text
    name =  response2
    check = [name, boy]
    listToStr = ' '.join([str(elem) for elem in check])
    return(listToStr)
...
def namegirl():
    response = requests.get('http://service2:5000/gender/girl').text
    girl = str(response)
    #genderid = int(reponse)
    #list = ['Boy', 'Girl', 'Neutral']
    #gender = list[genderid]
    ...
    response2 = requests.get('http://service3:5000/namebygirl').text
    name =  response2
#    check = [name,girl]
    check = str(name + ' ' + girl)
  #  listToStr = ' '.join([str(elem) for elem in check])
